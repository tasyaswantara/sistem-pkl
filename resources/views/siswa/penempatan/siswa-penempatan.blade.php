...
<x-admin-layout>
    <div>
        ...
        @if (session('success'))
            <div
                class="mb-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 animate-fade-up">
                {{ session('success') }}
            </div>
        @endif
        ...
        @php
            ...
            $pilihanLabel =
                $pilihan === PilihanSiswa::REKOMENDASI->value
                    ? 'Rekomendasi'
                    : ($pilihan === PilihanSiswa::USULAN_LAIN->value
                        ? 'Usulan Lain'
                        : ($pilihan === PilihanSiswa::LANGSUNG->value
                            ? 'Penempatan Langsung'
                            : 'Belum dipilih'));
            $pilihanIndustri =
                $pilihan === PilihanSiswa::USULAN_LAIN->value
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6 animate-fade-up">
            ...
            @endif
            @if (($penempatan?->jenis_penempatan ?? 'normal') === JenisPenempatan::LANGSUNG->value)
                ...
                <table class="w-full min-w-[700px]">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Industri</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Nilai
                                Preferensi</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($rekomendasi as $row)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-sm text-gray-700">
                                    <span
                                        class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-purple-100 text-purple-700 text-xs font-semibold">
                                        {{ $row->peringkat }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-700">{{ $row->industri?->nama_industri ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-700">
