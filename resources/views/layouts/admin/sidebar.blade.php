...
<div x-cloak :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
    ...
    <div class="flex items-center justify-center mt-8">
        <div class="flex items-center gap-3">
            <div
                class="w-12 h-12 rounded-lg bg-white/10 flex items-center justify-center overflow-hidden border border-white/20">
                <img src="{{ asset('assets/images/logo serviam.jpg') }}" alt="Logo" class="w-full h-full object-cover">
            </div>
    ...
    <!-- MENU -->
    <nav class="flex-1 px-3 py-6 space-y-1" x-data="{ activeHash: window.location.hash }" x-init="window.addEventListener('hashchange', () => activeHash = window.location.hash)">
            ...
            @if ($hasChildren)
                ...
                {{ request()->routeIs($menu['route'])
                    ...
                    : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                        ...
                        <svg class="w-4 h-4 text-white/60 transition-transform" :class="open ? 'rotate-180' : ''"
                    ...
                    <div x-show="open" x-collapse class="mt-1 space-y-1 ml-6 border-l border-white/10 pl-4">
                        @foreach ($menu['children'] as $child)
                            @php
                                ...
                                $childUrl = $childParams
                                    ? route($child['route'], $childParams)
                                    : route($child['route']);
                                if ($childHash) {
                                    $childUrl .= '#' . $childHash;
                                }
                                $isActiveChild =
                                    $child['route'] === 'admin.data-pengguna'
                                        ? request()->routeIs('admin.data-pengguna') &&
                                            request('role') === ($childParams['role'] ?? null)
                                        : ($child['route'] === 'admin.penempatan'
                                            ? request()->routeIs('admin.penempatan') &&
                                                request('tab', 'konfigurasi') === ($childParams['tab'] ?? null)
