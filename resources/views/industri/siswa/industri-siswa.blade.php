    ...
    <div x-data="{ openId: null, reportId: null }">
        <div class="mb-8">
            ...
            <p class="text-gray-500 text-sm max-w-2xl">
                Kelola siswa yang melamar, atur jadwal wawancara, dan kirim laporan keluhan kinerja siswa.
            </p>
        </div>
                ...
                <table class="w-full min-w-[1350px]">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Jadwal
                                Wawancara</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Berkas Siswa
                            </th>
                        ...
                        @forelse ($penempatanList as $row)
                            @php
                                ...
                                $bpjsUrl = $row->siswa?->bpjs_link
                                    ? (\Illuminate\Support\Str::startsWith($row->siswa->bpjs_link, [
                                        'http://',
                                        'https://',
                                    ])
                                        ? $row->siswa->bpjs_link
                                ...
                                $kartuUrl = $row->siswa?->kartu_pelajar_link
                                    ? (\Illuminate\Support\Str::startsWith($row->siswa->kartu_pelajar_link, [
                                        'http://',
                                        'https://',
                                    ])
                                ...
                                $statusClass = match ($row->status) {
                                    ...
                                    PenempatanStatus::PROSES_WAWANCARA->value
                            ...
                            <tr class="hover:bg-gray-50 align-top">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700">{{ $row->siswa?->jurusan?->nama ?? '-' }}
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                    ...
                                    <div class="flex flex-col gap-1">
                                        @if ($bpjsUrl)
                                            <a href="{{ $bpjsUrl }}" target="_blank"
                                                class="text-emerald-700 hover:underline text-xs">BPJS</a>
                                        @endif
                                        @if ($kartuUrl)
                                            <a href="{{ $kartuUrl }}" target="_blank"
                                                class="text-emerald-700 hover:underline text-xs">Kartu Pelajar</a>
                                        @endif
                                        @if ($cvUrl)
                                            <a href="{{ $cvUrl }}" target="_blank"
                                                class="text-emerald-700 hover:underline text-xs">CV</a>
                                        @endif
                                        @if ($portofolioLinks->isNotEmpty())
                                            <div class="text-xs text-gray-500">
                                                @foreach ($portofolioLinks as $idx => $link)
                                                    <a href="{{ $link }}" target="_blank"
                                                        class="text-emerald-700 hover:underline">Portfolio-{{ $idx + 1 }}</a>
                                                    @if (!$loop->last)
                                                        ,
                                                    @endif
                                                @endforeach
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700">
                                    @if ($row->laporan_industri)
                                        <div class="text-xs text-gray-500">
                                            {{ \Illuminate\Support\Str::limit($row->laporan_industri, 80) }}</div>
                                        <div class="text-xs text-emerald-600 mt-1">
                                            Status:
                                            {{ ucfirst($row->laporan_status ?? LaporanStatus::MENUNGGU->value) }}
                                        </div>
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    @php
                                        $decisionLabel = match ($row->status) {
                                        ...
                                        $decisionBadgeClass = match ($row->status) {
                                            PenempatanStatus::DITERIMA_INDUSTRI->value => 'border-emerald-200 bg-emerald-50 text-emerald-700',
                                            PenempatanStatus::TIDAK_LOLOS_INDUSTRI->value => 'border-rose-200 bg-rose-50 text-rose-700',
                                            default => 'border-gray-200 bg-gray-50 text-gray-600',
                                        };
                                        $acceptButtonClass = $row->status === PenempatanStatus::DITERIMA_INDUSTRI->value
                                            ? 'border-emerald-600 bg-emerald-600 text-white'
                                            : 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100';
                                        $rejectButtonClass = $row->status === PenempatanStatus::TIDAK_LOLOS_INDUSTRI->value
                                    ...
                                    <div class="w-[220px] space-y-3">
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold {{ $decisionBadgeClass }}">
                                                    {{ $decisionLabel }}
                                                </span>
                                            </div>
                                        </div>
                                        <form method="POST" action="{{ route('industri.siswa.status', $row->id) }}">
                                            ...
                                            <div class="grid grid-cols-2 gap-2">
                                                <button type="submit"
                                                    name="status"
                                                    value="{{ PenempatanStatus::DITERIMA_INDUSTRI->value }}"
                                                    class="rounded-lg border px-3 py-2 text-xs font-semibold transition {{ $acceptButtonClass }}">
                                                    Terima
                                                </button>
                                                <button type="submit"
                                                    name="status"
                                                    value="{{ PenempatanStatus::TIDAK_LOLOS_INDUSTRI->value }}"
                                                    class="rounded-lg border px-3 py-2 text-xs font-semibold transition {{ $rejectButtonClass }}">
                                                    Tidak Lolos
                                                </button>
                                            </div>
                                        </form>
                                        <div class="space-y-2 border-t border-gray-200 pt-3">
                                            <button type="button"
                                                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50"
                                                @click="openId = openId === {{ $row->id }} ? null : {{ $row->id }}">
                                                Atur Jadwal Wawancara
                                            </button>
                                            <button type="button"
                                                class="w-full rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700 hover:bg-rose-100"
                            ...
                            <tr x-show="openId === {{ $row->id }}" x-cloak class="bg-gray-50">
                                <td colspan="7" class="px-4 py-4">
                                    <div class="mb-3 text-xs text-gray-500">
                                        Gunakan formulir ini untuk mengatur jadwal wawancara siswa.
                                    </div>
                                    <form method="POST" action="{{ route('industri.siswa.jadwal', $row->id) }}"
                                        class="grid grid-cols-1 md:grid-cols-4 gap-3">
                                        @csrf
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-gray-700 mb-1.5">Tanggal</label>
                                        ...
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-gray-700 mb-1.5">Catatan</label>
                            ...
                            <tr x-show="reportId === {{ $row->id }}" x-cloak class="bg-gray-50">
                                <td colspan="7" class="px-4 py-4">
                                    <div class="mb-3 text-xs text-gray-500">
                                        Gunakan formulir ini untuk membuat laporan keluhan terkait kinerja siswa selama
                                        PKL.
                                    </div>
                                    <form method="POST" action="{{ route('industri.siswa.laporan', $row->id) }}"
                                        class="grid grid-cols-1 md:grid-cols-5 gap-3">
                                        @csrf
                                        <div class="md:col-span-4">
                                            <label class="block text-xs font-medium text-gray-700 mb-1.5">Isi Laporan
                                                Keluhan</label>
                                            <textarea name="laporan" rows="3" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
